...
- Note (Sammy T.): original author refers to 
...
def save_excel_file_path():
    ...
    root.filename = filedialog.asksaveasfilename(initialdir=getcwd(), \
                                                 ...
                                                 filetypes=(("Excel files", "*.xlsx"), \
                                                            ("all files", "*.*")))
    if root.filename:
        if not root.filename.endswith('.xlsx'):
            return root.filename + '.xlsx'
        else:
            print('Error: file is already written with .xlsx')
            return root.filename
...
# define cusip whitespace stripper here
...
def starter_check(des):
    # function is concerned with Title of Class file (in terms of a 13F report worksheet)
    # in other words, "issue column" refers to security type(?)
    ...
    starters = ["CALL", "PUT", "DEBT", "NAMEN", "*W", "ORDINARY", "RIGHT", "WBI",
                ...
                "SHS CL", "COM CL", "COM CLASS", " ORD","COM STK", "COM UNIT", "USD ORD", "ORD SH", "SHS CLASS",
                "UNIT COM", "ADR", "ADS", "COM", "UNIT", "ORD", "SHS", "CLASS", "S&P", "UT", "CL B"]
    # issue: if there are stocks for the quarter, which are uploaded into the report, and which do not have 
    #        representation in starters (if the securities keyword isn't in the list), then it's possible
    #        for the report to be awkwardly formatted (Name of Issuer col containing chars beloning to 
    #        securities in the Title of Class col)
    # issue is relative to both starter_check & end_check
...
def end_check(des):
    # function is concerned with ending letters/characters of Name of Issuer
    # (which is the column header in a 13F report worksheet; this is the Issuer Name column in a 13F pdf)
    ...
    ends = ["CORP N", "CORP NEW", "INC N", "INC NEW", "PLC", "LTD", "INC", "CORP", "HLDGS I", "HLDGS II", "HLDGS III",
            "HLDGS", "FD TR", "ETF TR", "BRH", "L P", " LP", "S A", "FD I", "FD II",
            "FD III", "ETF TR", "FD T", "TRADED FD", "FD I", "FD II", "FD III", "FDS", "FD", "TRUST", "TR"]
    # note: if you find that certain keywords don't 'work', check for duplicates
...
def format_workbook(path, count, xlsx_count):
    ...
    # rename opening worksheet (maybe should be final report)
    ws.title = 'SEC 13F List'
...
# rename CUSIP column to Cusip (for matching)
df = df.rename(columns={'CUSIP': 'Cusip'})

# remove_white_space
save_path = save_excel_file_path()
df.to_excel(save_path) # do we want to save the 13F securities as independent excel file 


# Format the Excel file
format_workbook(save_path, count, len(final_table))

# https://www.bing.com/images/search?view=detailV2&ccid=0rqgFtPe&id=517F5A4FFE4A7D9CE0D149E8AC13766F6E020BA6&thid=OIP.0rqgFtPeRBs1KJSq-AlyUAHaJQ&mediaurl=https%3a%2f%2fnativecanadianarts.com%2fwp-content%2fuploads%2f2017%2f07%2fWoodland-James-Jacko-2015-In-the-Spirit-of-Honouring-Our-Ancestors-1.jpg&cdnurl=https%3a%2f%2fth.bing.com%2fth%2fid%2fR.d2baa016d3de441b352894aaf8097250%3frik%3dpgsCbm92E6zoSQ%26pid%3dImgRaw%26r%3d0&exph=1300&expw=1040&q=anishinaabe+artists&simid=608038280361681321&FORM=IRPRST&ck=83F9F6B30FC0DB1F7A9C5175F0D55862&selectedIndex=0&itb=0&idpp=overlayview&ajaxhist=0&ajaxserp=0

# Requirements:
# - scrapes SEC 13F PDF for information
# - formats info into excel
# - formatting involves: removing whitespace from CUSIP column (so CUSIPs can be matched) 